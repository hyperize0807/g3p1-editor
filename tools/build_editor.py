...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .fld.efld{grid-template-columns:90px minmax(0,1fr) minmax(0,1.5fr)}
  .efld .edesc{color:var(--fg);font-size:12px;line-height:1.4;align-self:center;background:#202329;border:1px solid var(--line);border-radius:5px;padding:5px 8px;min-height:31px}
  .efld .edesc.empty{color:#666}
  .efld .edesc .ts{color:var(--warn);font-weight:600;margin-right:6px}
...
function edescHtml(id){   // 장비 우측 상시 설명: 무기면 TS/SS 강조 + 설명
  if(!id) return {empty:true, html:'(없음 / 빈 슬롯)'};
  const w=isWeapon(id), d=itemDesc(id);
  let h = w ? '<span class="ts">TS:'+itemTS(id)+' SS:'+itemSS(id)+'</span>' : '';
  h += d ? escA(d) : (w ? '' : '(설명 없음)');
  return {empty:false, html:h};
}
function equipField(label,off){   // 장비 슬롯: 라벨 + 드롭다운 + 우측 상시 설명
  const v=u16(off), e=edescHtml(v);
  return '<div class="fld efld">'+
    '<label>'+label+'</label>'+
    '<select data-off="'+off+'" data-size="2" data-kind="sel" data-item="1" title="'+escA(itemTip(v))+'">'+opt("Item",v,true)+'</select>'+
    '<div class="edesc'+(e.empty?' empty':'')+'">'+e.html+'</div>'+
  '</div>';
}
function renderEdit(){
  if(cur<0)return;
  ...
  '<div class="grp"><h3>장비 <span class="tag ok">확정</span></h3>'+
    equipField("무기 1",off+F.weap1)+
    equipField("무기 2",off+F.weap2)+
    equipField("방어구",off+F.armor)+
    equipField("장신구 1",off+F.acc1)+
    equipField("장신구 2",off+F.acc2)+
  '</div>'+
  ...
  '</div>';
  e.querySelectorAll('[data-off]').forEach(el=>el.addEventListener('change',()=>{
    applyField(el);
    if(el.dataset.item){
      const id=+el.value; el.title=itemTip(id);
      const box=el.closest('.fld'), ed=box&&box.querySelector('.edesc');
      if(ed){ const r=edescHtml(id); ed.className='edesc'+(r.empty?' empty':''); ed.innerHTML=r.html; }
    }
  }));

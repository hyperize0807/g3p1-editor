...
<header>
  ...
  <span id="verinfo" class="badge" style="display:none;background:var(--ok)"></span>
  <span id="vermismatch" class="hint" style="color:var(--warn)"></span>
...
// ===== 세이브 버전 (변조방지 체크섬 가중치로 판별) =====
// 1.03+ : 가중치 2*(i&3)+1 = 1,3,5,7 / 1.04 : 2*(i&3)+3 = 3,5,7,9 (모듈러스 32000, 끝 2바이트 LE)
// 두 패턴 어느 것에도 맞지 않으면 알 수 없는 버전 → 로드 거부(편집 차단).
const VERSIONS={'1.03+':1,'1.04':3};
let saveVersion=null;          // 로드 시 감지된 버전 키
function calcChecksum(raw,base){ let acc=0; for(let i=0;i<raw.length-2;i++){ acc=(acc+raw[i]*(2*(i&3)+base))%32000; } return acc; }
function detectVersion(raw){
  if(raw.length<4) return null;
  const stored=raw[raw.length-2]|(raw[raw.length-1]<<8);
  for(const k in VERSIONS){ if(calcChecksum(raw,VERSIONS[k])===stored) return k; }
  return null;
}
function updateVerMismatch(){
  const sv=document.getElementById('savever'), m=document.getElementById('vermismatch');
  m.textContent=(saveVersion && sv.value!==saveVersion)? '⚠ 로드한 '+saveVersion+'와(과) 다른 버전으로 저장합니다' : '';
}
function updateVerUI(){
  const vi=document.getElementById('verinfo'), vr=document.getElementById('saververap'), sv=document.getElementById('savever');
  if(!saveVersion){ vi.style.display='none'; vr.style.display='none'; document.getElementById('vermismatch').textContent=''; return; }
  vi.textContent='감지: '+saveVersion+' 버전'; vi.style.display='';
  sv.value=saveVersion; vr.style.display=''; updateVerMismatch();
}
...
function opt(table,val,desc){
  const t=TABLES[table]||{};
  const useDesc = desc===true && table==='Item';   // 아이템 드롭다운: 옵션 텍스트에 설명(무기는 TS/SS) 표기
  const lab=k=>{ let s=k+' — '+(t[k]!==undefined?t[k]:'(미정의)');
    if(useDesc){ const tip=itemTip(+k); if(tip) s+='  ·  '+tip; }
    return useDesc? escA(s) : s; };
  let s='<option value="'+val+'">'+lab(val)+'</option>';
...
function selField(label,off,table,size){
  ...
  return '<div class="fld"><label>'+label+' <span class="off">@0x'+off.toString(16)+'</span></label>'+
   '<select data-off="'+off+'" data-size="'+(size||2)+'" data-kind="sel"'+extra+'>'+opt(table,v,true)+'</select></div>';
}
...
function jobTokenHtml(t,isReq){
  ...
  return '<span class="atoken">'+nm+lv+'</span>';
}
function jobCardHtml(j){
  ...
  return '<div class="jobcard"><div><span class="jn">'+escA(j.name)+'</span>'+
    ...
    '<div class="jlearn"><span class="lbl">습득 가능 어빌리티</span>'+learn+'</div></div>';
}
...
function loadBuf(arr){
  ...
  if(!ver){                                   // 1.03+/1.04 외 패턴 → 안전을 위해 로드 거부
    buf=null; saveVersion=null;
    drop.innerHTML='<b style="color:#e6b6b6">⚠ 지원하지 않는 세이브 버전입니다.</b><br>'+
      '<span class="hint">변조방지 체크섬이 분석된 <b>1.03+ / 1.04</b> 패턴 중 어느 것과도 일치하지 않습니다. '+
      '파일이 손상되었거나 지원 범위 밖 버전일 수 있어, 안전을 위해 편집을 중단합니다.<br>('+escA(fileName)+')</span><br>'+
      '<button id="reopen" style="margin-top:10px">다른 .sav 열기</button>';
    drop.style.display='block';
    document.querySelector('main').style.display='none';
    document.getElementById('save').disabled=true;
    document.getElementById('invbtn').disabled=true;
    document.getElementById('reopen').onclick=()=>document.getElementById('file').click();
    updateVerUI();
    return;
  }
  saveVersion=ver;
...
function doSave(){
  ...
  // 변조방지 체크섬 재계산: raw 바이트[0..len-3]에 버전별 가중치 적용 후 %32000, 끝 2바이트(LE)에 기록
  // 1.03+ = 1,3,5,7 / 1.04 = 3,5,7,9. 저장 버전 선택값을 사용(미선택 시 1.03+).
  const base=VERSIONS[document.getElementById('savever').value] || 1;
  let acc=0;
  for(let i=0;i<out.length-2;i++){ acc=(acc + out[i]*(2*(i&3)+base))%32000; }
...
document.getElementById('savever').onchange=updateVerMismatch;
...
document.addEventListener('click',e=>{
  ...
  if(c){ const isel=c.closest('.isel'), menu=isel.querySelector('.isel-menu'), wasOpen=!menu.hidden;
    ...
    if(!wasOpen){
      ...
      menu.hidden=false;
      const q=isel.querySelector('.isel-q'); q.value=''; filterItemMenu(isel); q.focus({preventScroll:true});
...
// 메뉴 내부 스크롤은 무시(목록 스크롤 가능), 바깥(페이지/패널) 스크롤 시에만 닫기
window.addEventListener('scroll',e=>{ const t=e.target; if(t&&t.closest&&t.closest('.isel-menu')) return; closeItemMenus(); },true);
